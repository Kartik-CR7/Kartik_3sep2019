...
from MyDemoProject.settings import DEFAULT_EMAIL_TO
from MyDemoProject.settings import EMAIL_HOST_USER
...
def like_request(request):
    ...
    cur.execute("select ifnull(sum(final_tab.bin_cnt),0) from mydemoapp_bt_imageupload left join (select t2.button_id,t2.sess_response,t2.flag,case when t2.flag = 'O' then 1 when t2.flag = 'E' then 0 end as bin_cnt from  (select t1.button_id,t1.sess_response,t1.flag from (select distinct case when (count(1) over(partition by button_id,sess_response))% 2 <> 0 then 'O' when (count(1) over(partition by button_id,sess_response))% 2 = 0 then 'E' end as flag,sess_response,button_id from mydemoapp_like )t1 order by sess_response desc)t2) as final_tab on mydemoapp_bt_imageupload.image_id = final_tab.button_id where mydemoapp_bt_imageupload.Image_id = '" + btn_id + "'" + "group by mydemoapp_bt_imageupload.Image_id order by Image_id desc")
...
def TravelImageRefresh(request):
    ...
    # cur_image.execute("select final_tab.button_id,mydemoapp_bt_imageupload.Image,sum(final_tab.bin_cnt) from (select t2.button_id,t2.sess_response,t2.flag,case when t2.flag = 'O' then 1 when t2.flag = 'E' then 0 end as bin_cnt from  (select t1.button_id,t1.sess_response,t1.flag from (select distinct case when (count(1) over(partition by button_id,sess_response))% 2 <> 0 then 'O' when (count(1) over(partition by button_id,sess_response))% 2 = 0 then 'E' end as flag,sess_response,button_id from mydemoapp_like )t1 order by sess_response desc)t2) as final_tab,mydemoapp_bt_imageupload where mydemoapp_bt_imageupload.image_id = final_tab.button_id group by final_tab.button_id order by button_id desc")
    cur_image.execute("select mydemoapp_bt_imageupload.Image_name,mydemoapp_bt_imageupload.Image_id,mydemoapp_bt_imageupload.Image,ifnull(sum(final_tab.bin_cnt),0)as cnt,ifnull(final_tab.flag,'N') as flag from mydemoapp_bt_imageupload left join (select t2.button_id,t2.sess_response,t2.flag,case when t2.flag = 'O' then 1 when t2.flag = 'E' then 0 end as bin_cnt from  (select t1.button_id,t1.sess_response,t1.flag from (select distinct case when (count(1) over(partition by button_id,sess_response))% 2 <> 0 then 'O' when (count(1) over(partition by button_id,sess_response))% 2 = 0 then 'E' end as flag,sess_response,button_id from mydemoapp_like )t1 order by sess_response desc)t2) as final_tab on mydemoapp_bt_imageupload.image_id = final_tab.button_id group by mydemoapp_bt_imageupload.Image_id order by Image_id desc")
    ...
    return render(request,'mydemoapp/TravelImageDisplay.html',{"media_url": MEDIA_URL,"qry_list": qry_list})
